...
####################################------------- subject Validation -------------##################################
// check subject is set or not 
if ($validate->required_VALIDATION($subject)) {
    $errors_for_subject = "subject Is Required";
}
// check subject characters is grater than 3 or not 
elseif ($validate->min_VALIDATION($subject)) {
    $errors_for_subject = "subject Characters Must Be Greater Than 3";
}
// check subject characters is grater than 3 or not
elseif ($validate->max_VALIDATION($subject)) {
    $errors_for_subject = "subject Characters Must Be Lower Than 25 ";
}
// to return to register page with array 
if (!empty($errors_for_subject)) {
    $_SESSION['errors_for_subject'] =  $errors_for_subject;
    $validate->redirect('../contact.php');
}
####################################------------- message Validation -------------##################################
// check subject is set or not 
if ($validate->required_VALIDATION($message)) {
    $errors_for_message = "message Is Required";
}
// check message characters is grater than 3 or not 
elseif ($validate->min_VALIDATION($message)) {
    $errors_for_message = "message Characters Must Be Greater Than 3";
}
// to return to register page with array 
if (!empty($errors_for_message)) {
    $_SESSION['errors_for_message'] =  $errors_for_message;
    $validate->redirect('../contact.php');
}
####################################------------- store informations in database file  -------------##################################
if (empty($errors_for_username) && empty($errors_for_email) && empty($errors_for_subject)&& empty($errors_for_phone)) {
    echo $user->Insert(['name' => "$name", 'email' => $email, 'subject' => "$subject", 'phone' => "$phone" , 'message'=>$message], "contacts");
    $_SESSION['success'] = "The Message Is Created Successfully";
    $validate->redirect('../contact.php');
} else {
    $validate->redirect('../contact.php');
    die;
